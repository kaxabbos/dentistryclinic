...
public class NotesCont extends Attributes {
    ...
    public String noteOrdering(@RequestParam Long doctorId, @RequestParam String date, @RequestParam String time, @PathVariable Long noteId) {
        Doctors doctor = doctorsRepo.getReferenceById(doctorId);
        Users client = getUser();
        Notes note = notesRepo.getReferenceById(noteId);

        Ordering ordering = new Ordering(doctor, client, note, date, time);
